...
class NeuralNetwork:
    ...
    def computeLoss(self, Y, predictions):
        ...
        # TODO 
        n, sl = Y.shape
        last_activation = predictions[-1]
        error = np.zeros([n, sl])
        error = Y*np.log(last_activation)
        error = (-1/n)*np.sum(error)
        return error
    ...
    def feedforward(self, X):
        ...
        for i in range(num_layers):
            Z = ((self.layers)[i]).forwardpass(activations[i])
            activations[i+1] = Z.copy()
        return activations
        #raise NotImplementedError
        # END TODO
    ...
    def backpropagate(self, activations, Y):
        ...
        # TODO
        num_layers = len(self.layers)
        #assumption: Y is of size n X sl
        last_activation = activations[-1]
        delta_ = -Y/last_activation
        for i in range(1,num_layers+1):
            delta_ = ((self.layers)[-i]).backwardpass(self.alpha, activations[-(i+1)], delta_)
        #raise NotImplementedError
        # END TODO